...
#include "ui_program_channel_simulation.h"

program_channel_simulation::program_channel_simulation(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::program_channel_simulation)
{
    ui->setupUi(this);
}

program_channel_simulation::~program_channel_simulation()
{
    delete ui;
}

void program_channel_simulation::on_pushButton_19_clicked()
{
    qDebug()<<"klik OK simulation than create database";
    back_to_home_2();
    create_database();
}

void program_channel_simulation::create_database()
{
	QSqlQuery query;
    ...
    query.prepare(QString("UPDATE ddu_ch SET cur_jenis_sensor='%2', cur_sensor_name='%3', cur_ser_baud='%5', cur_ser_parity='%6', cur_ser_data_bits='%7', cur_ser_stop_bit='%8', cur_ser_flow='%9', cur_protokol='%13', cur_freq_out='%14' where ch_id=%15;").arg(ambil_jenis).arg(ambil_nama).arg(ambil_baudrate).arg(ambil_parity).arg(ambil_databit).arg(ambil_stopbit).arg(ambil_flow).arg(ambil_output_value).arg(ambil_frekuensi).arg(id_channel_input));
    query.exec();

    qDebug()<<"Update Combobox simulation to database"<<query.lastQuery();

}

void program_channel_simulation::setup_program_channel_simulation(QString id_channel)
{
    qDebug()<<"masuk sinyal dari channel "<<id_channel;
	QSqlQuery query;

    query.prepare(QString("SELECT * FROM ddu_ch where ch_id = %1;").arg(15));
    query.exec();
    query.seek(0);
    qDebug()<<"last query take simulation database"<<query.lastQuery();
    ...
    else if(!QString::compare(nama_sensor, "Simulation"))
